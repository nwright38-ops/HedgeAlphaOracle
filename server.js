...
function checkPayment(req, res, next) {
  ...
  if (!paymentSig) {
    const amounts = { sentiment: '50000', fullalpha: '100000', premium: '200000' };
    const maxAmount = amounts[tier] || '50000';
    const resource = BASE_URL + req.path + (req.query.tier ? '?tier=' + tier : '');

    // x402 v2 standard format — matches x402scan strict schema
    return res.status(402).json({
      ...
      resource: {
        url: resource,
        description: 'HedgeAlphaOracle ' + tier + ' signal — real-time crypto & stock trading signals',
        mimeType: 'application/json'
      },
      extensions: {
        bazaar: {
          info: {
            input: { asset: 'BTC', tier: tier, timeframe: '1D' },
            output: {
              success: true,
              asset: 'BTC',
              tier: tier,
...
// x402scan discovery endpoint — no .json extension, version:1 schema
app.get('/.well-known/x402', (req, res) => {
  res.json({
    version: 1,
    resources: [
      BASE_URL + '/signal/BTC?tier=sentiment',
      BASE_URL + '/signal/BTC?tier=fullalpha',
      BASE_URL + '/signal/BTC?tier=premium'
    ],
    instructions: '# HedgeAlphaOracle\n\nReal-time crypto & stock trading signals via x402 micropayments on Base.\n\n## Tiers\n- sentiment: $0.05 (first query FREE)\n- fullalpha: $0.10\n- premium: $0.20\n\n## Usage\nReplace BTC with any supported asset: BTC, ETH, SOL, AAPL, NVDA, TSLA and more.\n\n## Provider\nNurse2Web3 — https://nurse2web3.com'
  });
});
